...
public HashSet<Usuario> retornaUsuariosEmprestaveis() {
    ...
    public void pagarDebitoUsuario (int id){
    ...
    public void incrementaQtdEmp (int id){
   ...
   public void cadastrarAluno (String nome, String telefone, String dataNascimento, String matriculaAluno, int anoAluno){
    ...
    public void listarLivros (){
    	for (Livro e : this.livros){
    		JOptionPane.showMessageDialog(null, "Titulo:"+e.getTitulo() +" \n Autor: "+e.getAutor()+"\n Editora: "+e.getEditora()+"\n " +
    ...
    public void renovaEmprestimo (int diasRenovados, int id){
    	
    	for (Emprestimo i : this.emprestimos){
    		if (i.getId() == id){
    			i.renovarEmprestimo(diasRenovados);
    		}
    	}
    	
    }
    
    public void livroDevolvido (int i){
    	
    	for (Livro e : this.livros){
    		
    		if (e.getId() == i){
    			int numExemp = e.getQtdVolumes();
    			numExemp++;
    			e.setQtdVolumes(numExemp);
    			break;
    ...
    public void finalizarEmprestimo (int i){
    	
    	for (Emprestimo e : this.emprestimos){
    		if (e.getId() == i){
    			int idLivro = e.getLivroEmprestimo().getId();
    			this.livroDevolvido(idLivro);
    ...
    public Livro returnLivroMaisEmprestado (){
    	...
    	for (Livro i : this.livros){
    		if (cont == 0){
    			maisEmp = i;
    		} else if (i.getQtdEmprestimos() > maisEmp.getQtdEmprestimos()) {
    			maisEmp = i;
    		}
    		cont++;
    	}
    ...
    public void relatorio (){
    	JOptionPane.showMessageDialog(null, "Livro mais emprestado: " +this.returnLivroMaisEmprestado().getTitulo());
    	
    	HashSet<Usuario> users = this.retornaUsuariosDevedores();
    	String listaDevedores = "";
    	for (Usuario i : users){
    		listaDevedores += i.getNome()+" \n";
    	}
    	JOptionPane.showMessageDialog(null, "Usuarios devedores:  \n"+listaDevedores);
